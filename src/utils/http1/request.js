...
//参考： https://gitee.com/chun_cheng/axios_package/tree/master
...
function request(config, { noToken, noLoading } = {}) {
  ...
  if (method == "GET" && config.isCache.time > 0) {
    ...
    let cacheObj = {}; // 保存可能会出现在 url 后的参数 // get请求参数可能存在于url中，所以单独处理一次
    if (/\?/.test(cacheUrl)) {
  ...
  } else if (method == "POST" && config.isCache.time > 0) {
    // 给post请求方法设置数据缓存
...
function fetch(
  ...
  method = "GET",
  data = {},
  { noToken, noLoading } = { noToken: true, noLoading: false },
  isCache = { time: 0 },
  responseType = "json"
) {
  // 获取请求类型
